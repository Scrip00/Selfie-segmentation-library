...
namespace mediapipe
{
  ...
  absl::Status BackgroundMaskingCalculator::RenderGpu(CalculatorContext *cc)
  {
    ...
    auto input_frame = absl::make_unique<mediapipe::ImageFrame>(mediapipe::ImageFormat::SRGB, background.cols, background.rows,
    ...
    // cv::resize(background, background, Size(img_tex.width(), img_tex.height()));
